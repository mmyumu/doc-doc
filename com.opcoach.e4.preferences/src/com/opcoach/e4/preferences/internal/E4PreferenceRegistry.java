...
public class E4PreferenceRegistry {
	...
	public PreferenceManager getPreferenceManager() {
			...
			if (elmt.getAttribute(ATTR_CLASS) != null) {
				...
				try {
					...
					if (!(object instanceof PreferencePage)) {
						logger.error(
								"Expected instance of PreferencePage: {0}",
								elmt.getAttribute(ATTR_CLASS));
						continue;
					}
		...
		while (!childrenNodes.isEmpty()) {
			for (String cat : Collections.unmodifiableSet(childrenNodes
					.keySet())) {
		...
		if (psProviders == null) {
			...
			for (IConfigurationElement elmt : registry
				...
				if ((isEmpty(classname) && isEmpty(objectId))
						|| (((classname != null) && classname.length() > 0) && ((objectId != null) && objectId
								.length() > 0))) {
					logger.warn("In extension "
							+ PREF_STORE_PROVIDER
